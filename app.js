const MAP_SIZE=6000,INITIAL_MASS=40,MIN_MASS=10,ABSORB_RATIO=1.1,PELLET_COUNT=3500,PELLET_MASS=1.6,PELLET_RADIUS=7,EJECT_MASS=8,EJECT_RADIUS=4,EJECT_SPEED=600,SPLIT_COOLDOWN=0.8,MERGE_DELAY=15,BASE_SPEED=1200,RADIUS_SCALE=4,ZOOM_MIN=0.45,ZOOM_MAX=1.30,GRID_STEP=50,ENEMY_START=24,BUG_DRAIN=0.12,BAT_DRAIN=0.05,ENEMY_RADIUS=22,SPLIT_LAUNCH=1200,MIN_SPEED=20,MAX_SPEED=700,REPULSE_K=0.6,ATTRACT_K=0.25,MERGE_RATE=0.45,KEBAB_COUNT=40,KEBAB_RADIUS=22,KEBAB_MASS=40,MAX_CELL_MASS=22500;
...
const state={player:null,food:[],ejected:[],enemies:[],kebabs:[],particles:[],cafardImg:null,kebabImg:null,friteImg:null,pelletQT:new Quad(0,0,MAP_SIZE,MAP_SIZE),enemyQT:new Quad(0,0,MAP_SIZE,MAP_SIZE),camera:{x:MAP_SIZE/2,y:MAP_SIZE/2,zoom:1},mouse:[MAP_SIZE/2,MAP_SIZE/2],running:false,difficulty:"normal",theme:"dark"};
function spawnFood(n=PELLET_COUNT){for(let i=0;i<n;i++){state.food.push({x:rnd(MAP_SIZE),y:rnd(MAP_SIZE),r:PELLET_RADIUS,m:PELLET_MASS,rot:Math.random()*Math.PI*2})}}
...
function start(){const name=nickEl.value.trim()||"Azadi";const imgEl=skinEl.files[0];state.difficulty=document.getElementById("difficulty").value;const themeEl=document.getElementById("theme");state.theme=themeEl?themeEl.value:"dark";document.body.classList.toggle("theme-light",state.theme==="light");document.body.classList.toggle("theme-dark",state.theme==="dark");menu.style.display="none";canvas.style.display="block";document.getElementById("hud").style.display="flex";const img=new Image();if(imgEl){const url=URL.createObjectURL(imgEl);img.src=url;img.onload=()=>URL.revokeObjectURL(url)}state.cafardImg=new Image();state.cafardImg.src="img/cafard.png";state.kebabImg=new Image();state.kebabImg.src="img/kebab.png";state.friteImg=new Image();state.friteImg.src="img/frite.png";
...
function handleCollisions(dt){
  ...
  for(const c of cells){
    ...
    for(const it of q1){
      ...
      if(d<rr+it.r){const nm=c.mass+it.m;c.mass=Math.min(MAX_CELL_MASS,nm);it.r=0}
    }
  ...
  state.player.cells=state.player.cells.filter(c=>c.mass>MIN_MASS);
}
...
function drawFood(){const c=state.camera;for(const p of state.food){const [x,y]=toScreen(p.x,p.y,c);const r=p.r*c.zoom;if(state.friteImg){ctx.save();ctx.translate(x,y);ctx.rotate(p.rot||0);ctx.drawImage(state.friteImg,-r,-r,r*2,r*2);ctx.restore()}else{ctx.fillStyle="#ffd27a";ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill()}}for(const p of state.ejected){const [x,y]=toScreen(p.x,p.y,c);ctx.fillStyle="#ffd84a";ctx.beginPath();ctx.arc(x,y,p.r*c.zoom,0,Math.PI*2);ctx.fill()}if(state.kebabImg){for(const k of state.kebabs){const [x,y]=toScreen(k.x,k.y,c);const r=k.r*c.zoom;ctx.drawImage(state.kebabImg,x-r,y-r,r*2,r*2)}}}
