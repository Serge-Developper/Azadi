const MAP_SIZE=6000,INITIAL_MASS=40,MIN_MASS=10,ABSORB_RATIO=1.1,PELLET_COUNT=1200,PELLET_MASS=1.6,PELLET_RADIUS=3,EJECT_MASS=8,EJECT_RADIUS=4,EJECT_SPEED=600,SPLIT_COOLDOWN=0.8,MERGE_DELAY=15,BASE_SPEED=300,RADIUS_SCALE=4,ZOOM_MIN=0.35,ZOOM_MAX=1.4,GRID_STEP=50,ENEMY_START=24,BUG_DRAIN=0.02,BAT_DRAIN=0.05,ENEMY_RADIUS=22,SPLIT_LAUNCH=1200,MIN_SPEED=60,MAX_SPEED=520,REPULSE_K=0.6,ATTRACT_K=0.25,MERGE_RATE=0.45;
...
class Enemy{constructor(x,y,type){this.x=x;this.y=y;this.type=type;this.vx=rnd(200)-100;this.vy=rnd(200)-100;this.r=ENEMY_RADIUS}update(dt){this.x+=this.vx*dt;this.y+=this.vy*dt;if(this.x<this.r||this.x>MAP_SIZE-this.r)this.vx*=-1;if(this.y<this.r||this.y>MAP_SIZE-this.r)this.vy*=-1;if(irnd(100)<2){this.vx=rnd(240)-120;this.vy=rnd(240)-120}}drainRate(){return this.type==="bat"?BAT_DRAIN:BUG_DRAIN}}
const state={player:null,food:[],ejected:[],enemies:[],particles:[],cafardImg:null,pelletQT:new Quad(0,0,MAP_SIZE,MAP_SIZE),enemyQT:new Quad(0,0,MAP_SIZE,MAP_SIZE),camera:{x:MAP_SIZE/2,y:MAP_SIZE/2,zoom:1},mouse:[MAP_SIZE/2,MAP_SIZE/2],running:false,difficulty:"normal"};
...
function rebuildQT(){state.pelletQT.clear();state.enemyQT.clear();for(const p of state.food)state.pelletQT.insert(p);for(const e of state.ejected)state.pelletQT.insert(e);for(const e of state.enemies)state.enemyQT.insert(e)}
function spawnDrainParticles(x,y,amt){const n=Math.min(14,Math.max(6,Math.round(amt/2)));for(let i=0;i<n;i++){state.particles.push({x,y,vx:(Math.random()*2-1)*220,vy:(Math.random()*2-1)*220,life:0.6,r:2+Math.random()*2,c:"#ff7a3b"})}}
...
function start(){const name=nickEl.value.trim()||"Azadi";const imgEl=skinEl.files[0];state.difficulty=document.getElementById("difficulty").value;menu.style.display="none";canvas.style.display="block";const img=new Image();if(imgEl){const url=URL.createObjectURL(imgEl);img.src=url;img.onload=()=>URL.revokeObjectURL(url)}state.cafardImg=new Image();state.cafardImg.src="img/cafard.png";
state.player=new Player(name,imgEl?img:null);const sx=rnd(MAP_SIZE-1000)+500,sy=rnd(MAP_SIZE-1000)+500;state.player.addCell(new Cell(sx,sy,INITIAL_MASS,colorFor(name),imgEl?img:null));spawnFood();spawnEnemies(ENEMY_START);state.running=true;lastT=now();loop()}
...
function handleCollisions(dt){
  ...
  for(const c of cells){
    ...
    for(const en of q2){
      ...
      if(d<rr+en.r){
        const rate=en.drainRate()*(state.difficulty==="hard"?1.6:1);
        const lose=c.mass*rate*dt;
        c.mass=Math.max(MIN_MASS,c.mass-lose);
        spawnDrainParticles(c.x,c.y,lose);
      }
    }
  }
}
function updateCamera(dt){const [cx,cy]=state.player.center();state.camera.x+= (cx-state.camera.x)*0.1;state.camera.y+= (cy-state.camera.y)*0.1;const tm=state.player.totalMass;const z=clamp(ZOOM_MAX-0.4*Math.log10(tm/INITIAL_MASS+1),ZOOM_MIN,ZOOM_MAX);state.camera.zoom+= (z-state.camera.zoom)*0.08}
function drawGrid(){const c=state.camera;const step=GRID_STEP*c.zoom;const ox=(W/2 - c.x*c.zoom)%step,oy=(H/2 - c.y*c.zoom)%step;ctx.strokeStyle="#1e1e1e";ctx.lineWidth=1;ctx.beginPath();for(let x=ox;x<W;x+=step){ctx.moveTo(x,0);ctx.lineTo(x,H)}for(let y=oy;y<H;y+=step){ctx.moveTo(0,y);ctx.lineTo(W,y)}ctx.stroke()}
function draw(){ctx.clearRect(0,0,W,H);drawGrid();drawMapBounds();drawFood();drawEnemies();drawParticles();drawPlayer()}
function drawMapBounds(){const c=state.camera;ctx.strokeStyle="#333";ctx.lineWidth=4;ctx.strokeRect(...toScreen(0,0,c),MAP_SIZE*c.zoom,MAP_SIZE*c.zoom)}
function drawFood(){const c=state.camera;for(const p of state.food){const [x,y]=toScreen(p.x,p.y,c);ctx.fillStyle=`hsl(${p.c} 70% 60%)`;ctx.beginPath();ctx.arc(x,y,p.r*c.zoom,0,Math.PI*2);ctx.fill()}for(const p of state.ejected){const [x,y]=toScreen(p.x,p.y,c);ctx.fillStyle="#ffd84a";ctx.beginPath();ctx.arc(x,y,p.r*c.zoom,0,Math.PI*2);ctx.fill()}}
function drawEnemies(){const c=state.camera;for(const e of state.enemies){const [x,y]=toScreen(e.x,e.y,c);const r=e.r*c.zoom;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);if(e.type==="bug"&&state.cafardImg){ctx.save();ctx.clip();ctx.drawImage(state.cafardImg,x-r,y-r,r*2,r*2);ctx.restore();ctx.strokeStyle="#3b2a1a";ctx.lineWidth=2;ctx.stroke()}else{ctx.fillStyle=e.type==="bat"?"#ff3b3b":"#54ff78";ctx.fill()}}}
